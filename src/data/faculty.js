export const faculty = [
    {
        id: "ana-elizabeth-almanza-cruz",
        name: "Dra. Ana Elizabeth Almanza Cruz",
        degree: "Doctor en Derecho Penal",
        bio: "Jueza de control, enjuiciamiento y ejecución penal especializada en adolescentes y violencia contra las mujeres en Jalisco.",
        fullBio: "Jueza de control, enjuiciamiento y ejecución penal especializada en adolescentes y violencia contra las mujeres en Jalisco.",
        /* education: [
            "Doctorado en Derecho Penal - UNAM",
            "Maestría en Juicios Orales - INACIPE",
            "Licenciatura en Derecho - Universidad Iberoamericana"
        ], */
        image: "/Dra._Ana_Elizabeth_Almanza_Cruz.avif"
    },
    {
        id: "juan-jose-rodriguez-velarde",
        name: "Mtro. Juan José Rodríguez Velarde",
        degree: "Maestría en Derechos Humanos",
        bio: "Juez de Control, Enjuiciamiento, Justicia para Adolescentes y Ejecución Penal del X Distrito Judicial en Tequila.",
        fullBio: "Juez de Control, Enjuiciamiento, Justicia para Adolescentes y Ejecución Penal del X Distrito Judicial en Tequila.",
        /* education: [
            "Maestría en Derechos Humanos - Ibero Ciudad de México",
            "Especialidad en Derecho Internacional - Universidad de Salamanca",
            "Licenciatura en Derecho - ITAM"
        ], */
        image: "/juan-jose-rodriguez-velarde.avif"
    },
    {
        id: "jose-de-jesus-angulo-aguirre",
        name: "Dr. José de Jesús Angulo Aguirre",
        degree: "Especialista en Derecho Corporativo",
        bio: "Magistrado en retiro del Tribunal Electoral del Estado de Jalisco, con trayectoria en justicia electoral.",
        fullBio: "Magistrado en retiro del Tribunal Electoral del Estado de Jalisco, con trayectoria en justicia electoral.",
        education: [
            "Especialidad en Derecho Corporativo - UP",
            "Licenciatura en Derecho - Escuela Libre de Derecho"
        ],
        image: "/jose-de-jesus-angulo-aguirre.avif"
    },
    {
        id: "jose-alberto-espinoza-herrera",
        name: "Mtro. José Alberto Espinoza Herrera",
        degree: "Doctora en Administración Pública",
        bio: "Juez Segundo de Ejecución Penal del Primer Distrito Judicial del Estado de Jalisco.",
        fullBio: "Juez Segundo de Ejecución Penal del Primer Distrito Judicial del Estado de Jalisco.",
        education: [
            "Doctorado en Administración Pública - Universidad Anáhuac",
            "Maestría en Políticas Públicas - CIDE",
            "Licenciatura en Ciencias Políticas - UNAM"
        ],
        image: "/jose-alberto-espinoza-herrera.avif"
    },
    {
        id: "ma-karina-dolores-rivas-gutierrez",
        name: "Dra. Ma Karina Dolores Rivas Gutierrez",
        degree: "Doctora en Administración Pública",
        bio: "Jueza tercero de Control, Enjuiciamiento y justicia Integral para Adolescentes",
        fullBio: "Jueza tercero de Control, Enjuiciamiento y justicia Integral para Adolescentes",
        education: [
            "Doctorado en Administración Pública - Universidad Anáhuac",
            "Maestría en Políticas Públicas - CIDE",
            "Licenciatura en Ciencias Políticas - UNAM"
        ],
        image: "/ma-karina-dolores-rivas-gutierrez.avif"
    },
    {
        id: "martha-temores-sanchez",
        name: "Mtra. Martha Temores Sánchez",
        degree: "Doctora en Administración Pública",
        bio: "Maestra en Derecho con amplia trayectoria en el Poder Judicial de Jalisco. Docente en derecho penal y sistema acusatorio.",
        fullBio: "Maestra en Derecho con amplia trayectoria en el Poder Judicial de Jalisco. Docente en derecho penal y sistema acusatorio.",
        education: [
            "Doctorado en Administración Pública - Universidad Anáhuac",
            "Maestría en Políticas Públicas - CIDE",
            "Licenciatura en Ciencias Políticas - UNAM"
        ],
        image: "/martha-temores-sanchez.avif"
    }
];
